...
```bash
# Mode: full (all checks) | medium (skip onboarding, default) | low (minimal)
_MODE=$(~/.claude/skills/gstack/bin/gstack-config get mode 2>/dev/null || echo "medium")
echo "MODE: $_MODE"
...
if [ "$_MODE" != "low" ]; then
  ...
  for _PF in $(find ~/.gstack/analytics -maxdepth 1 -name '.pending-*' 2>/dev/null); do
    if [ -f "$_PF" ]; then
      if [ "${_TEL:-off}" != "off" ] && [ -x "~/.claude/skills/gstack/bin/gstack-telemetry-log" ]; then
...
Set once with `gstack-config set mode <mode>`. Default: `medium`.

| Mode | What runs | Token cost vs full |
|------|-----------|-------------------|
| `full` | Every check, all onboarding prompts, learnings, routing setup | 100% |
| `medium` | Update check, proactive/repo detection, telemetry logging — skips all onboarding prompts | ~40% |
| `low` | Session tracking + branch only — skips everything else | ~10% |

To override for a single run, temporarily set: `gstack-config set mode low` then reset after.
